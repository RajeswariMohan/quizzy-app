...
if docker compose -f docker-compose.test.yml ps --status running postgres 2>/dev/null | grep -q postgres; then
  echo "==> Applying 007 via docker (port 5433)..."
  docker compose -f docker-compose.test.yml exec -T postgres \
    psql -U quizzy -d quizzy -v ON_ERROR_STOP=1 < database/migrations/007_user_sessions.sql
else
  echo "==> Applying 007 via local psql (see .env for host/port)..."
  PGPASSWORD="${DATABASE_PASSWORD:-quizzy}" psql \
    ...
    -v ON_ERROR_STOP=1 \
    -f database/migrations/007_user_sessions.sql
fi

echo "==> Done. user_sessions table is ready."
